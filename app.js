...
const cloudFns = getFunctions(fbApp);
...
window.doLogin = function() {
  ...
  signInWithEmailAndPassword(auth, email, pass)
    .then(() => {
      ...
      try { httpsCallable(cloudFns, 'resetFailedLogin')(); } catch (e) {}
    })
    .catch(async err => {
      showLoader(false);
      ...
      if (['auth/invalid-credential','auth/wrong-password','auth/user-not-found','auth/invalid-email'].includes(err.code)) {
        ...
        if (isMobileNum && (err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password')) {
          try {
            const result = await httpsCallable(cloudFns, 'recordFailedLogin')({ mobile });
            const d = result.data || {};
            if (d.locked) {
...
window.adminSetMemberPassword = async function(uid, name) {
  ...
  showLoader(true);
  try {
    await httpsCallable(cloudFns, 'adminSetPassword')({ uid, newPassword: finalPassword });
    showLoader(false);
...
window.adminUnlockMemberAccount = async function(uid, name) {
  ...
  showLoader(true);
  try {
    await httpsCallable(cloudFns, 'adminUnlockMember')({ uid, newPassword: newPassword || undefined });
    showLoader(false);
