...
window.tryUpiApp = function(scheme) {
  ...
  // This VPA is a personal/SHG account, not a verified merchant. UPI apps
  // reject a deep link that pre-fills an amount to a non-merchant payee
  // ("VPA or UPI is not available"). So we open the app with the payee only
  // and the user enters the amount in the app (shown + copyable in the modal).
  const openParams = pendingUpiParams
    .split('&')
    .filter(kv => !/^am=/i.test(kv))
    .join('&');
  window.location.href = `${scheme}${openParams}`;
};
...
function playFireworks() {
  ...
  function frame() {
    ...
    if (Date.now() - start < 3400 || parts.length || rockets.length) requestAnimationFrame(frame);
    else done();
  }
...
window.payNow = function() {
  ...
  const params = `pa=${encodeURIComponent(UPI)}&pn=${encodeURIComponent(PAYEE)}&am=${amtFmt}&cu=INR&tn=${encodeURIComponent(note)}`;
