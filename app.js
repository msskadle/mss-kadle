...
function listenAllMembers() {
  ...
  unsubMembers = onSnapshot(collection(db, 'members'), snap => {
    ...
    items.forEach(m => {
      ...
      // Keep the member-visible 'directory' mirror in sync (no DOB / mobile / email).
      if (m.status === 'approved') {
        setDoc(doc(db, 'directory', m.id), {
          name: m.name || '', nameKn: m.nameKn || '', role: m.role || 'member',
          status: m.status || 'approved', currentLoan: Number(m.currentLoan || 0),
          monthlyHistory: m.monthlyHistory || {}, profilePic: m.profilePic || ''
        }, { merge: true }).catch(() => {});
      }
    });
...
function loadMembersTab() {
  ...
  if (isTreasurer) {
    // Treasurer/admin: read full records (incl. DOB + mobile) and refresh the
    // public directory mirror, which deliberately excludes DOB / mobile / email.
    getDocs(collection(db, 'members')).then(snap => {
      membersDirCache = [];
      snap.forEach(d => {
        const m = d.data();
    ...
    // Regular members: read the directory mirror only — no DOB, no mobile.
    getDocs(collection(db, 'directory')).then(snap => {
      membersDirCache = [];
      snap.forEach(d => membersDirCache.push({ id: d.id, ...d.data() }));
      finish();
    }).catch(() => showToast('Failed to load members'));
  }
}
