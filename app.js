...
window.showAdminTab = function(tab, btn) {
  ...
  btn.classList.add('active');
  ['members', 'payments', 'requests', 'ann', 'gallery', 'donations', 'burst'].forEach(t => {
...
window.submitBulkRecordPayment = async function() {
  ...
  if (!confirm(`Record for ${ids.length} member(s) — ${monthLabel}?\nFee: ₹${feeNum}\nLoan repaid: ₹${loanNum}`)) return;
  ...
  showToast(failCount ? `✅ ${successCount} recorded, ⚠️ ${failCount} failed` : `✅ Recorded for ${successCount} member(s) · ದಾಖಲಾಯಿತು`);
  loadTotalFeeCollected();
};
...
window.resetAllMemberPayments = async function() {
  ...
  const typed = prompt(`Type RESET to confirm wiping payment data for all ${members.length} members:`);
  ...
  // Clear local caches so UI reflects the wipe immediately
  ...
  showLoader(false);
  showToast(failCount ? `✅ ${successCount} reset, ⚠️ ${failCount} failed` : `✅ Reset complete for ${successCount} member(s) · ಅಳಿಸಲಾಗಿದೆ`);
  loadTotalFeeCollected();
};
