...
window.goTab = function(t) {
  ...
  if (t === 'admin' && isAdmin) loadAssocData();
};
...
function initMpayTab() {
  ...
  document.getElementById('mpay-edit-total-fee-btn').style.display = isAdmin ? 'inline-block' : 'none';
...
function renderPastSavings(profile) {
  ...
  if (isAdmin) {
    // Admin can set only if not already approved (one-time)
...
window.showAdminTab = function(tab, btn) {
  ...
  btn.classList.add('active');
  ['members', 'requests', 'ann', 'gallery', 'assoc'].forEach(t => {
...
// Stored in settings/association doc with fields:
// totalSavings, fd, loanGiven, interest, extLoanGiven, extLoanReceived

function loadAssocData() {
  getDoc(doc(db, 'settings', 'association')).then(snap => {
    const d = snap.exists() ? snap.data() : {};
    const totalSavings = Number(d.totalSavings || 0);
    const fd = Number(d.fd || 0);
    const loanGiven = Number(d.loanGiven || 0);
    const interest = Number(d.interest || 0);
    const extLoanGiven = Number(d.extLoanGiven || 0);
    const extLoanReceived = Number(d.extLoanReceived || 0);
...
window.editAssocField = function(field, label) {
  getDoc(doc(db, 'settings', 'association')).then(snap => {
    ...
    // FD changes adjust Total Savings automatically
    if (field === 'fd') {
      const oldFd = Number(d.fd || 0);
      const oldTotal = Number(d.totalSavings || 0);
      const diff = num - oldFd; // positive = more moved into FD
      update.totalSavings = Math.max(0, oldTotal - diff);
    }
