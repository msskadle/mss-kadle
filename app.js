...
function loadAssocData() {
  ...
  Promise.all([
    getDoc(doc(db, 'settings', 'totals')),
    getDoc(doc(db, 'settings', 'association')),
    getDocs(collection(db, 'members'))
  ]).then(([totalsSnap, assocSnap, membersSnap]) => {
    // Total member fee collected (mirrors Member Pay tab logic)
    let totalFee;
    const totalsData = totalsSnap.exists() ? totalsSnap.data() : {};
    if (totalsData.totalFeeOverride !== undefined && totalsData.totalFeeOverride !== null) {
      totalFee = Number(totalsData.totalFeeOverride);
    } else {
      totalFee = 0;
    ...
    // Loan given to members = sum of all members' currentLoan
    let loanGiven = 0;
    membersSnap.forEach(d => { loanGiven += Number((d.data().currentLoan) || 0); });
    ...
    const totalSavings = Math.max(0, totalFee - fd);
