...
window.goTab = function(t) {
  ...
  if (t === 'admin') { if (isAdmin) { listenAdminAnn(); listenAdminApprovals(); } }
};
...
// Can now also move the entry to a different month (e.g. payment was logged under the wrong month).
window.editMonthEntry = function(memberId, month) {
  ...
  let confirmMsg = `Update entry for ${m.name}?\nFee: ₹${Number(feeVal)}\nLoan repaid: ₹${Number(loanVal)}`;
  confirmMsg += monthChanged ? `\nMonth: ${label} → ${newMonthLabel}` : `\nMonth: ${label}`;
...
window.showAdminTab = function(tab, btn) {
  ...
  btn.classList.add('active');
  ['members', 'approvals', 'requests', 'ann', 'gallery', 'donations', 'burst'].forEach(t => {
...
window.adminBulkAddMembers = async function() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const dob = parts[2] || '';
    const pass = parts[3] || '';
    if (!name || !mobile) { errEl.textContent = `Line ${i + 1}: name and mobile are required`; errEl.classList.add('show'); return; }
    if (!/^\d{10}$/.test(mobile)) { errEl.textContent = `Line ${i + 1}: "${mobile}" is not a valid 10-digit mobile number`; errEl.classList.add('show'); return; }
    const finalPass = (pass && pass.length >= 6) ? pass : mobile;
    rows.push({ line: lines[i], name, mobile, dob, pass: finalPass });
  }
  ...
  if (!confirm(`Add ${rows.length} member(s)? They will be auto-approved.`)) return;

  showLoader(true);
  progEl.style.display = 'block';

  const secondaryApp = initializeApp(firebaseConfig, 'bulkAdd_' + Date.now());
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const uid = cred.user.uid;
      await setDoc(doc(db, 'members', uid), {
        name: r.name, nameKn: r.name, dob: r.dob || '', mobile: r.mobile, email,
...
window.resetAllMemberPayments = async function() {
  if (!(isAdmin || isTreasurer)) { showToast('Admin/Treasurer only'); return; }
