...
function initMpayTab() {
  ...
  const monthInput = document.getElementById('mpay-req-month');
...
function renderMpayTable(profile) {
  ...
  const balanceLoan = Math.max(0, currentLoan - totalReturned);
...
// ---- Member: raise payment request ----
window.submitMpayRequest = function() {
  const type = document.getElementById('mpay-req-type').value;
  const month = document.getElementById('mpay-req-month').value;
  const amount = document.getElementById('mpay-req-amount').value;

  if (!month) { showToast('Select a month · ಮಾಸ ಆಯ್ಕೆಮಾಡಿ'); return; }
  if (!amount || isNaN(amount) || Number(amount) < 1) { showToast('Enter a valid amount · ಮೊತ್ತ ನಮೂದಿಸಿ'); return; }
  ...
  addDoc(collection(db, 'requests'), {
    ...
    memberName: currentProfile.name,
    title: `${type} - ${month}`,
    body: `${type} payment of ${fmtMoney(amount)} for ${month}`,
    paymentType: type,
    month,
    ...
    showLoader(false);
    document.getElementById('mpay-req-amount').value = '';
  ...
  unsubMpayRequests = onSnapshot(query(collection(db, 'requests'), orderBy('createdAt', 'desc')), snap => {
    ...
    list.innerHTML = items.map(r => `
      <div class="req-card">
        ...
        <div class="req-title">${r.memberName} · ${r.paymentType}</div>
        <div class="req-body">${r.month} — ${fmtMoney(r.amount)}</div>
...
window.approveMpayRequest = async function(reqId) {
  ...
  try {
    ...
    if (r.paymentType === 'Loan Repayment') {
      entry.loanReturned = Number(entry.loanReturned || 0) + Number(r.amount);
    } else {
      entry.fee = Number(entry.fee || 0) + Number(r.amount);
    }
    history[r.month] = entry;

    await updateDoc(memberRef, { monthlyHistory: history });
    ...
    if (mpaySelectedMemberId === r.memberId) {
      const updatedMember = { id: r.memberId, ...member, monthlyHistory: history };
