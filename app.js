  ...
  unsubCountdown = onSnapshot(query(collection(db, 'countdownEvents'), orderBy('order', 'asc')), snap => {
    ...
    snap.forEach(d => {
      ...
      card.innerHTML =
        ...
        `</div>` +
        (isAdmin ? `<div style="display:flex;gap:.4rem;margin-top:.5rem;"><button onclick="editCountdownEvent('${id}')" style="flex:1;font-size:.55rem;padding:.25rem;border:none;border-radius:6px;background:rgba(255,255,255,.15);color:#fff;cursor:pointer;">✏️ Edit</button><button onclick="deleteCountdownEvent('${id}')" style="flex:1;font-size:.55rem;padding:.25rem;border:none;border-radius:6px;background:rgba(255,80,80,.3);color:#fff;cursor:pointer;">🗑</button></div>` : '');
