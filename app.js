  ...
  unsubMembers = onSnapshot(collection(db, 'members'), snap => {
    ...
    list.innerHTML = items.map(m => `
      <div class="member-row">
        <div class="mr-av">${m.role === 'admin' ? '⚙️' : (m.role === 'treasurer' ? '💰' : (m.status === 'onboarding' ? '🕐' : '👤'))}</div>
        <div class="mr-mid">
          <strong>${m.name} ${m.role === 'admin' ? '<span class="admin-badge">ADMIN</span>' : (m.role === 'treasurer' ? '<span class="admin-badge" style="background:var(--nv);">TREASURER</span>' : '')} ${m.status === 'onboarding' ? '<span class="admin-badge" style="background:#e67e22;">ONBOARDING</span>' : ''}</strong>
