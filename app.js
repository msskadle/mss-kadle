...
function renderMembersList(items) {
  ...
  list.innerHTML = items.map((m, i) => {
    // Compute fee collected from monthlyHistory
    const history = m.monthlyHistory || {};
    let feeCollected = 0;
    Object.values(history).forEach(h => { feeCollected += Number(h.fee || 0); });
    // Add approved past savings
    if (m.pastSavings && m.pastSavings.status === 'approved') {
      feeCollected += Number(m.pastSavings.amount || 0);
    }
    ...
    return `<div class="card" style="margin-bottom:.6rem;padding:.8rem;">
      <div style="display:flex;align-items:center;gap:.7rem;">
        <div style="font-size:1.6rem;min-width:2rem;text-align:center;">${avatar}</div>
        <div style="flex:1;">
          <div style="font-size:.82rem;font-weight:800;color:var(--txt);">${m.name} ${roleBadge}</div>
          ${m.nameKn ? `<div style="font-size:.72rem;color:var(--sub);font-family:'Noto Sans Kannada',sans-serif;">${m.nameKn}</div>` : ''}
        </div>
        <div style="font-size:.75rem;font-weight:800;color:var(--nv);">#${String(i+1).padStart(3,'0')}</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.4rem;margin-top:.6rem;">
        <div style="background:var(--bg);border-radius:8px;padding:.4rem .5rem;text-align:center;">
          <div style="font-size:.6rem;color:var(--sub);margin-bottom:.1rem;">DOB · ಹುಟ್ಟಿದ ದಿನ</div>
          <div style="font-size:.68rem;font-weight:700;color:var(--txt);">${dobDisplay}</div>
        </div>
        <div style="background:#e8f5e9;border-radius:8px;padding:.4rem .5rem;text-align:center;">
          <div style="font-size:.6rem;color:var(--sub);margin-bottom:.1rem;">Fee Collected · ಶುಲ್ಕ</div>
          <div style="font-size:.68rem;font-weight:700;color:var(--green);">${fmtMoney(feeCollected)}</div>
        </div>
        <div style="background:${loanBalance > 0 ? '#fdecea' : '#e8f5e9'};border-radius:8px;padding:.4rem .5rem;text-align:center;">
          <div style="font-size:.6rem;color:var(--sub);margin-bottom:.1rem;">Loan Balance · ಸಾಲ ಬಾಕಿ</div>
          <div style="font-size:.68rem;font-weight:700;color:${loanBalance > 0 ? 'var(--red)' : 'var(--green)'};">${loanBalance > 0 ? fmtMoney(loanBalance) : 'Nil'}</div>
        </div>
      </div>
    </div>`;
  }).join('');
...
window.filterMembersList = function() {
  ...
  if (!q) { renderMembersList(membersDirCache); return; }
  const filtered = membersDirCache.filter(m =>
    (m.name || '').toLowerCase().includes(q) || (m.nameKn || '').includes(q)
  );
