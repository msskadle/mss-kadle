...
// Direct intent/app-based UPI payments are rejected by the receiver's bank
// for this VPA ("receiver not accepting payments on this UPI ID"), so we
// always show a scannable QR code instead — same flow as a merchant QR.
...
function showUpiQr(params) {
  ...
  document.getElementById('upi-modal-limit').textContent = `Max ₹${UPI_MAX_AMOUNT.toLocaleString('en-IN')} per transaction · ಪ್ರತಿ ಪಾವತಿಗೆ ಗರಿಷ್ಠ ₹${UPI_MAX_AMOUNT.toLocaleString('en-IN')}`;
...
window.payNow = function() {
  ...
  setTimeout(() => showToast('Scan QR to pay · QR ಸ್ಕ್ಯಾನ್ ಮಾಡಿ'), 300);
};
...
window.memPay = async function() {
  ...
  const params = `pa=${UPI}&am=${amtFmt}&cu=INR`;
  launchUpiPayment(params);
  setTimeout(() => showToast('Payment recorded. Scan QR to pay · ಪಾವತಿ ದಾಖಲಾಗಿದೆ'), 300);
};
