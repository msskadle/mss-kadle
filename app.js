...
window.submitBulkRecordPayment = async function() {
  ...
  const confirmMsg = `Record for ${ids.length} member(s) — ${months.length} month(s): ${monthLabels}?\nFee per month: ₹${feeNum}\nLoan repaid per month: ₹${loanNum}\nTotal per member: ₹${(feeNum + loanNum) * months.length}`;
  if (!confirm(confirmMsg)) return;
  ...
  showToast(totalFail ? `✅ ${totalSuccess} updates recorded, ⚠️ ${totalFail} failed` : `✅ Recorded for ${ids.length} member(s) across ${months.length} month(s) · ದಾಖಲಾಯಿತು`);
  loadTotalFeeCollected();
};
