...
const PHONE_DOMAIN = 'msskadle.app'; // fake domain to convert phone -> email for Firebase Auth

function phoneToEmail(mobile) {
  return mobile.replace(/\D/g, '') + '@' + PHONE_DOMAIN;
}
...
let isAdmin = false;
let unsubMembers = null, unsubAnns = null, unsubMyReqs = null, unsubMyTxns = null, unsubGallery = null, unsubAllReqs = null, unsubAllTxns = null, unsubMyProfile = null;
...
window.doLogin = function() {
  ...
  if (!mobile || !pass) {
    errEl.textContent = 'Please enter mobile number and password · ಮೊಬೈಲ್ ಮತ್ತು ಪಾಸ್‌ವರ್ಡ್ ನಮೂದಿಸಿ';
    errEl.classList.add('show');
    return;
  }
  if (!/^\d{10}$/.test(mobile)) {
    errEl.textContent = 'Enter a valid 10-digit mobile number · ಮೌಲ್ಯಯುತ ಮೊಬೈಲ್ ಸಂಖ್ಯೆ ನಮೂದಿಸಿ';
    errEl.classList.add('show');
    return;
  }
...
window.doForgotPass = function() {
  const mobile = document.getElementById('li-email').value.trim();
  if (!/^\d{10}$/.test(mobile)) { showToast('Enter your 10-digit mobile number first · ಮೊಬೈಲ್ ಸಂಖ್ಯೆ ನಮೂದಿಸಿ'); return; }
  showToast('Password reset is not available via mobile login. Contact admin · ಆಡಮಿನ್ ಸಂಪರ್ಕಿಸಿ: 9880166445');
};
...
window.doRegister = function() {
  ...
  createUserWithEmailAndPassword(auth, email, pass)
    .then(async (cred) => {
      ...
      hideReg();
      document.getElementById('li-email').value = mobile;
      signOut(auth);
    })
    .catch(err => {
      showLoader(false);
...
function enterApp() {
  ...
  document.getElementById('pid').textContent = currentProfile.mobile || currentProfile.email;
...
function exitApp() {
  ...
  [unsubMembers, unsubAnns, unsubMyReqs, unsubMyTxns, unsubGallery, unsubAllReqs, unsubAllTxns, unsubMyProfile].forEach(u => { if (u) u(); });
}
...
window.payNow = function() {
  ...
  const url = `upi://pay?pa=${UPI}&pn=${encodeURIComponent(PAYEE)}&am=${amt}&cu=INR&tn=Donation`;
...
window.memPay = function() {
  ...
  addDoc(collection(db, 'transactions'), {
    ...
    const note = currentPType === 'Loan Repayment' ? 'Loan+Repayment' : 'Membership+Fee';
    const url = `upi://pay?pa=${UPI}&pn=${encodeURIComponent(PAYEE)}&am=${amt}&cu=INR&tn=${note}`;
    window.location.href = url;
...
function listenMyTransactions() {
  ...
  unsubMyTxns = onSnapshot(query(collection(db, 'transactions'), orderBy('createdAt', 'desc')), snap => {
    ...
    snap.forEach(d => {
      const t = d.data();
      if (t.memberId === currentUser.uid) items.push(t);
    });
    ...
    if (items.length === 0) {
      ...
      list.innerHTML = items.map(t => {
        ...
        const statusLabel = t.status === 'confirmed' ? '✅ Confirmed' : (t.status === 'rejected' ? '❌ Rejected' : '⏳ Pending verification');
        return `<div class="txn-item">
          <div class="txn-icon ${isLoan ? 'loan' : ''}">${isLoan ? '🏦' : '💰'}</div>
          <div class="txn-mid"><strong>${t.type}</strong><span>${fmtDateTime(t.createdAt)} · ${statusLabel}</span></div>
...
// Refresh member's own profile totals live (after admin confirms a payment)
function listenMyProfile() {
  if (unsubMyProfile) unsubMyProfile();
  unsubMyProfile = onSnapshot(doc(db, 'members', currentUser.uid), snap => {
    if (!snap.exists()) return;
    const m = snap.data();
    currentProfile = { id: currentUser.uid, ...m };
    document.getElementById('m-total').textContent = fmtMoney(m.totalSavings);
    document.getElementById('m-fee').textContent = fmtMoney(m.membershipFeePaid);
    document.getElementById('m-loan').textContent = fmtMoney(m.loanRepaid);
  });
}
    ...
    list.innerHTML = items.map(m => `
      <div class="member-row">
        ...
        <div class="mr-mid">
          ...
          <span>📱 ${m.mobile || m.email} ${m.dob ? '· DOB: ' + m.dob : ''}</span>
        </div>
        ${m.status === 'pending' ?
...
function listenAllTransactions() {
  if (unsubAllTxns) unsubAllTxns();
  unsubAllTxns = onSnapshot(query(collection(db, 'transactions'), orderBy('createdAt', 'desc')), snap => {
    ...
    if (items.length === 0) {
      list.innerHTML = '<div class="empty-state"><div class="ei">💳</div><div class="et">No payments yet</div></div>';
      return;
    }

    list.innerHTML = items.map(t => {
      const isLoan = t.type === 'Loan Repayment';
      const statusLabel = t.status === 'confirmed' ? '✅ Confirmed' : (t.status === 'rejected' ? '❌ Rejected' : '⏳ Pending');
      return `<div class="txn-item" style="flex-wrap:wrap;align-items:flex-start;">
        <div class="txn-icon ${isLoan ? 'loan' : ''}">${isLoan ? '🏦' : '💰'}</div>
        <div class="txn-mid">
          <strong>${t.memberName}</strong>
          <span>${t.type} · ${fmtDateTime(t.createdAt)} · ${statusLabel}</span>
        </div>
        <div class="txn-amt ${isLoan ? 'loan' : ''}">${fmtMoney(t.amount)}</div>
        ${t.status === 'pending' ? `
          <div style="display:flex;gap:.5rem;width:100%;margin-top:.6rem;">
            <button onclick="confirmTxn('${t.id}','${t.memberId}','${t.type}',${t.amount})" style="flex:1;background:var(--green);color:white;border:none;padding:.5rem;border-radius:6px;font-size:.7rem;font-weight:700;cursor:pointer;">✓ Confirm Payment</button>
            <button onclick="rejectTxn('${t.id}')" style="flex:1;background:var(--red);color:white;border:none;padding:.5rem;border-radius:6px;font-size:.7rem;font-weight:700;cursor:pointer;">✕ Reject</button>
          </div>` : ''}
      </div>`;
    }).join('');
  });
...
window.confirmTxn = function(txnId, memberId, type, amount) {
  showLoader(true);
  // Mark transaction confirmed
  updateDoc(doc(db, 'transactions', txnId), { status: 'confirmed', confirmedAt: serverTimestamp() })
    .then(async () => {
      // Update member's cumulative totals in Firestore
      const memberRef = doc(db, 'members', memberId);
      const snap = await getDoc(memberRef);
      if (snap.exists()) {
        const m = snap.data();
        const updates = { totalSavings: (m.totalSavings || 0) + Number(amount) };
        if (type === 'Loan Repayment') updates.loanRepaid = (m.loanRepaid || 0) + Number(amount);
        else updates.membershipFeePaid = (m.membershipFeePaid || 0) + Number(amount);
        await updateDoc(memberRef, updates);
      }
      showLoader(false);
      showToast('✅ Payment confirmed · ಪಾವತಿ ದೃಢೀಕರಿಸಲಾಗಿದೆ');
    })
    .catch(() => { showLoader(false); showToast('Failed to confirm'); });
};

window.rejectTxn = function(txnId) {
  updateDoc(doc(db, 'transactions', txnId), { status: 'rejected' })
    .then(() => showToast('Payment rejected · ಪಾವತಿ ತಿರಸ್ಕರಿಸಲಾಗಿದೆ'))
    .catch(() => showToast('Failed'));
};
...
window.showAdminTab = function(tab, btn) {
  ...
  btn.classList.add('active');
  ['members', 'payments', 'requests', 'ann', 'gallery'].forEach(t => {
