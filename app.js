...
function launchUpiPayment(params) {
  if (isIOS()) {
    // iOS: no generic upi:// handler, but each UPI app has its own scheme
    // that accepts the same query params. Show a chooser of app buttons.
    pendingUpiParams = params;
    showIosAppChooser(params);
  } else {
    // Android & others: plain upi:// passes all params correctly to the
    // selected UPI app. If WhatsApp opens instead of GPay/PhonePe, the user
    // should clear WhatsApp as default handler: Settings > Apps > WhatsApp
...
function showIosPayDetails(params) {
...
// iOS: app-specific custom URL schemes (each app maps the same UPI params
// onto its own scheme). If the app isn't installed, iOS silently does
// nothing (no error shown), so we just attempt the launch.
const IOS_UPI_SCHEMES = [
  { id: 'gpay',   label: 'Google Pay',  icon: '🟢', scheme: 'gpay://upi/pay?' },
  { id: 'phonepe',label: 'PhonePe',     icon: '🟣', scheme: 'phonepe://pay?' },
  { id: 'paytm',  label: 'Paytm',       icon: '🔵', scheme: 'paytmmp://pay?' },
  { id: 'bhim',   label: 'BHIM',        icon: '🟠', scheme: 'bhim://pay?' }
];

window.tryIosUpiApp = function(scheme) {
  if (!pendingUpiParams) return;
  window.location.href = `${scheme}${pendingUpiParams}`;
};

function showIosAppChooser(params) {
  const p = parseQueryParams(params);
  document.getElementById('upi-modal-amt').textContent = `₹${p.am}`;
  document.getElementById('upi-modal-id').textContent = p.pa;
  document.getElementById('upi-modal-name').textContent = p.pn;
  ...
  const wrap = document.getElementById('upi-ios-apps');
  if (wrap) {
    wrap.innerHTML = IOS_UPI_SCHEMES.map(a =>
      `<button onclick="tryIosUpiApp('${a.scheme}')" class="upi-app-btn">${a.icon} ${a.label}</button>`
    ).join('');
  }
  openModal('upi-modal');
}
...
window.payNow = function() {
  ...
  const params = `pa=${UPI}&am=${amtFmt}&cu=INR`;
